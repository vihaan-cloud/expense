<?php  
  class Expense extends Base {
    ...
    public function dtwise($UserId, $FROM, $TO, $locId){
      $stmt = $this->pdo->prepare("SELECT * FROM expense WHERE (Date >= :fromdate AND Date <= (:todate + INTERVAL 1 DAY)) AND UserId = :user AND location_id=:location_id ORDER BY Date");
      $stmt->bindParam(":user", $UserId, PDO::PARAM_INT);
      $stmt->bindParam(":location_id", $locId, PDO::PARAM_INT);
    ...
    public function mthwise($UserId, $FROM, $TO, $locId){
      $stmt = $this->pdo->prepare("SELECT * FROM expense WHERE (Date >= :fromdate AND Date <= (:todate + INTERVAL 1 MONTH)) AND UserId = :user AND location_id=:location_id ORDER BY Date ");
      $stmt->bindParam(":user", $UserId, PDO::PARAM_INT);
      $stmt->bindParam(":location_id", $locId, PDO::PARAM_INT);
      $stmt->bindParam(":fromdate", $FROM, PDO::PARAM_STR); 
      $stmt->bindParam(":todate", $TO, PDO::PARAM_STR);
    ...
    public function yrwise($UserId, $FROM, $TO, $locId){
      $stmt = $this->pdo->prepare("SELECT * FROM expense WHERE (EXTRACT(year FROM Date) >= :fromdate AND EXTRACT(year FROM Date) <= (:todate)) AND UserId = :user AND location_id=:location_id ORDER BY Date");
      $stmt->bindParam(":user", $UserId, PDO::PARAM_INT);
      $stmt->bindParam(":location_id", $locId, PDO::PARAM_INT);
      $stmt->bindParam(":fromdate", $FROM, PDO::PARAM_STR); 
      $stmt->bindParam(":todate", $TO, PDO::PARAM_STR);
    ...
    public function allexp($UserId) {
      $stmt = $this->pdo->prepare("SELECT a.*, b.location_name FROM expense AS a LEFT JOIN locations as b ON a.location_id=b.id WHERE a.UserId = :UserId ORDER BY Date");
      $stmt->bindParam(":UserId", $UserId, PDO::PARAM_INT);
      $stmt->execute();
    ...
    public function getAllLocation(){
      $stmt = $this->pdo->prepare("SELECT * FROM locations WHERE is_active=1");
      $stmt->execute();
      $all_row = $stmt->fetchall(PDO::FETCH_OBJ);
      return $all_row;
    }
